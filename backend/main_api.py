...
async def create_measure(measure_data: MeasureCreate, database: Session = Depends(get_db)) -> Measure:
    ...
    db_measure = Measure(
        error=measure_data.error,        unit=measure_data.unit,        value=measure_data.value,        uncertainty=measure_data.uncertainty,        observation_id=measure_data.observation,        metric_id=measure_data.metric,        measurand_id=measure_data.measurand        )
...
async def bulk_create_measure(items: list[MeasureCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_measure = Measure(
                error=item_data.error,                unit=item_data.unit,                value=item_data.value,                uncertainty=item_data.uncertainty,                observation_id=item_data.observation,                metric_id=item_data.metric,                measurand_id=item_data.measurand            )
            database.add(db_measure)
            database.flush()  # Get ID without committing
...
async def create_model(model_data: ModelCreate, database: Session = Depends(get_db)) -> Model:
    ...
    db_model = Model(
        data=model_data.data,        source=model_data.source,        licensing=model_data.licensing.value,        pid=model_data.pid,        dataset_id=model_data.dataset,        project_id=model_data.project        )
...
async def bulk_create_model(items: list[ModelCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_model = Model(
                data=item_data.data,                source=item_data.source,                licensing=item_data.licensing.value,                pid=item_data.pid,                dataset_id=item_data.dataset            )
            database.add(db_model)
            database.flush()  # Get ID without committing
...
async def create_feature(feature_data: FeatureCreate, database: Session = Depends(get_db)) -> Feature:
    ...
    db_feature = Feature(
        min_value=feature_data.min_value,        feature_type=feature_data.feature_type,        max_value=feature_data.max_value,        features_id=feature_data.features,        date_id=feature_data.date,        project_id=feature_data.project        )
...
async def bulk_create_feature(items: list[FeatureCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_feature = Feature(
                min_value=item_data.min_value,                feature_type=item_data.feature_type,                max_value=item_data.max_value,                features_id=item_data.features,                date_id=item_data.date            )
            database.add(db_feature)
            database.flush()  # Get ID without committing
...
async def create_legalrequirement(legalrequirement_data: LegalRequirementCreate, database: Session = Depends(get_db)) -> LegalRequirement:
    ...
    db_legalrequirement = LegalRequirement(
        standard=legalrequirement_data.standard,        principle=legalrequirement_data.principle,        legal_ref=legalrequirement_data.legal_ref,        project_1_id=legalrequirement_data.project_1        )
...
async def bulk_create_legalrequirement(items: list[LegalRequirementCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_legalrequirement = LegalRequirement(
                standard=item_data.standard,                principle=item_data.principle,                legal_ref=item_data.legal_ref,                project_1_id=item_data.project_1            )
            database.add(db_legalrequirement)
            database.flush()  # Get ID without committing
...
async def create_tool(tool_data: ToolCreate, database: Session = Depends(get_db)) -> Tool:
    ...
    db_tool = Tool(
        source=tool_data.source,        licensing=tool_data.licensing.value,        version=tool_data.version,        name=tool_data.name        )
...
async def bulk_create_tool(items: list[ToolCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_tool = Tool(
                source=item_data.source,                licensing=item_data.licensing.value,                version=item_data.version,                name=item_data.name            )
            database.add(db_tool)
            database.flush()  # Get ID without committing
